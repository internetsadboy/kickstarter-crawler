  ...
  funding_fundingGoal: function(html) {
	...
	pledges_data: function(html) {
		...
		for(var i = 0; i < numPledges; i++) {
			tempJSON = {};
			tempJSON['amount'] = Number($('.NS-projects-rightcol-rewards h5 span').eq(i).text().trim().replace(/,/g, '').split('$')[1]);
			numBackers = $('.NS-projects-rightcol-rewards .num-backers').eq(i).text().trim().split(' ')[0];
			tempJSON['num_backers'] = Number(numBackers);
			totalBackers = $('#backers_count').attr('data-backers-count');
			tempJSON['pledge_percentage'] = Number((parseFloat(numBackers/totalBackers)*100).toFixed(2));
			tempJSON['delivery_month'] = $('.delivery-date').eq(i).text().replace(/\n/g, '').split(':')[1].split(' ')[0];
			tempJSON['delivery_year'] = Number($('.delivery-date').eq(i).text().replace(/\n/g, '').split(':')[1].split(' ')[1]);
		...
		request(project.url, function(err, res, body) {
			if(!err) {
				...
				if(Object.keys(errors).length === 0) {
					if(log) utils.toColors(successes);
