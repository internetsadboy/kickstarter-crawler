...
function Project(options) {
  // config stuff
  log = options !== undefined ? options.log || false : false;
  hits = options !== undefined ? options.hits || false : false;
  fields = options !== undefined ? options.fields || false : false;
...
var data_points = {
  ...
  facebook_url: function(html) {
    var $ = cheerio.load(html);
    var link = $('.creator-details .facebook-connected a').attr('href');
  ...
  pledges_limited: function(html) {
    ...
    $ = cheerio.load(html);
    numLimited = 0
    $('.limited-number').each(function(i, elem) { numLimited++; });
    return (numLimited === 0) ? false : true;
  },
  pledges_percentLimited: function(html) {
    var numLimited, numPledges, $;
...
Project.prototype.parse = function(body) {
  ...
  if(project.fields.isEmpty()) {
    for(var i in FIELDS) {
      project.fields.enqueue(FIELDS[i]);
    }
  }

  var successes = {}, errors = {}, i;
  ...
  request(project.url, function(err, res, body) {
    if(!err) {
      ...
      if(hits) {
        ...
        h = Object.keys(result.successes).length;
        m = Object.keys(result.errors).length;
        c = (h/(h+m)).toFixed(3)*100;
        d = {
          complete: c,
          miss: result.errors,
